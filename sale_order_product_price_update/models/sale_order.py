    ...
    def action_sale_order_product_price_update_wizard(self):
        ...
        for order_line in self.order_line:
            if order_line.product_id and str(order_line.product_id.id) != deposit_product_id:
                product_lst_price_with_pricelist = order_line.order_id.pricelist_id._get_product_price(order_line.product_id, 1, order_line.product_uom)
                if order_line.product_id.list_price_last_updated \
                        and order_line.product_id.list_price_last_updated > order_line.create_date\
